...
/* a precedure take userName as parameter and show his checking date as output */
...
/
/* end of procedure program */




/* A function will take 2 parameter and return the smallest one */

/* 4 */
create or replace function smallfun(num1 in number,num2 in number)
return number
is
smallest number;
begin
...
num1 := 1546;
num2 := 1156;
ans := smallfun(num1,num2);
dbms_output.put_line('Smallest number between ' ||num1 || ' and ' || num2 || ' is : ' || ans);
end;
/
/* end of function */
...
/*I will use an array which will store the room type from room table*/

/* 5 */
set serveroutput on
declare
roomType room.room_type%type;
type namearray is varray(10) of room.room_type%type;
...
select room_type into roomType from room where room_id=x;
a_name.extend();
a_name(counter) := roomType;
counter := counter+1;
end loop;

counter := 1;
while counter<=a_name.count loop
dbms_output.put_line('Room Type : ' || a_name(counter));
counter := counter+1;
end loop;
...
declare
roomType room.room_type%type;
type namearray is varray(5) of room.room_type%type;
a_name namearray := namearray('Room1','Room2','Room3','Room4','Room5');
counter number;
begin
counter := 1;
for x in 1..5 loop
select room_type into roomType from room where room_id=x;
if roomType = 'Double Room' then
dbms_output.put_line('Room is of double type');
else
dbms_output.put_line('Room is of single type');
end if;
end loop;
end;
/
